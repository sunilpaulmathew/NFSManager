    ...
    public void exportMagiskLog(Context context) {
        new AsyncTasks() {
            ...
            public void onPreExecute() {
                ...
                mProgressMessage.setText(context.getString(R.string.exporting, new File(context.getExternalFilesDir("logs"),"magisk.log")) + "...");
            }

            @Override
            public void doInBackground() {
                Utils.runCommand("sleep 2");
                Utils.copy("/cache/magisk.log", new File(context.getExternalFilesDir("logs"),"magisk.log").getAbsolutePath());
            }

            @Override
            public void onPostExecute() {
                mProgressLayout.setVisibility(View.GONE);
                if (Utils.exist(new File(context.getExternalFilesDir("logs"),"magisk.log").getAbsolutePath())) {
                    new MaterialAlertDialogBuilder(context)
                            ...
                            .setPositiveButton(context.getString(R.string.share), (dialog, id) -> {
                                Uri uriFile = FileProvider.getUriForFile(context,
                                        BuildConfig.APPLICATION_ID + ".provider", new File(context.getExternalFilesDir("logs"),"magisk.log"));
